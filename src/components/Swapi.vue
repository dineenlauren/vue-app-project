<template>
  <div class="swapi">
    <h1>Star Wars</h1>     
    <p>
      <router-link v-bind:to="{ name: 'Swapi' }" class="active">Swapi</router-link>
      &bull;  
      <router-link v-bind:to="{ name: 'HPotter' }">Potter</router-link>              
    </p>



    <form v-on:submit.prevent="getNames">
        <p>Enter character name: <input type="text" v-model="query">
    ...
    <div class="results-container">
      ...
      <h2 v-if="results.results && results.results.length > 0">{{ results.results.length }} Results Found</h2>
