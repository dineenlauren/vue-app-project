...
<!-- NAV LINKS -->
    <p>
      <router-link v-bind:to="{ name: 'Swapi' }">Swapi</router-link>
      &bull;     
      <router-link v-bind:to="{ name: 'HPotter' }" class="active">Potter</router-link>      
    </p>
        ...
        <ul v-if="results && results.length > 0" :class="newStyle"> 
          <transition-group name="bounce" tag="div" appear>         
              <li v-for="result in results" class="item" v-bind:key="result.name">
                ...
                <p class="attribute">Blood Status:
                <br>
                {{ result.bloodStatus }}</p>
                <p class="attribute">      
                Role: 
                <br>    
                {{ result.role }}</p>               
